...
time_cols = [col for col in friday_alighters.columns 
             if col not in ['ID', 'NLC', 'ASC', 'Station', 'Mode', 'Line', 'Dir', 'Platform', 'Total', 
...
if time_slot in time_cols:
    ...
    station_values = friday_alighters.groupby('Station')[time_slot].sum().sort_values(ascending=False)
